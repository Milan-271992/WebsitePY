...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      status: 'ok',
      smtpConnected: true,
      emailUser: process.env.EMAIL_USER || 'Xtisupport@gmail.com',
    ...
    res.status(200).json({
      status: 'warning',
      smtpConnected: false,
      message: 'Server is running, but Gmail SMTP verification failed. Note: Gmail requires a 16-character Google App Password (not standard account password).',
      error: error.message,
    });
  }
});
...
app.post('/api/send-enquiry', async (req, res) => {
  ...
  try {
    ...
    // Try sending customer auto-reply (catch independently so admin mail success isn't blocked if customer mail typos)
    try {
    ...
    if (error.code === 'EAUTH' || (error.response && error.response.includes('535'))) {
      tip = 'Gmail authentication failed. Please generate a 16-character Google App Password in Google Account > Security > 2-Step Verification > App Passwords, and set EMAIL_PASS in your .env file.';
    }
